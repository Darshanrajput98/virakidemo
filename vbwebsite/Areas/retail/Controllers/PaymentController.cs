...
namespace vbwebsite.Areas.retail.Controllers
{
    public class PaymentController : Controller
    {
        ...
        public PaymentController(IRetPaymentService paymentservice, ICommonService commonservice, IRetCustomerService customerservice)
        {
        ...
        [HttpPost]
        public PartialViewResult ViewPaymentPendingList(RetPaymentListResponse model)
        {
            ViewBag.CashOption = _commonservice.GetAllRetCashOption();
            List<RetPaymentListResponse> objModel = _paymentservice.GetAllPaymentList(model);
            return PartialView(objModel);
        }

        [HttpPost]
        public ActionResult SavePayment(List<RetPaymentListResponse> data)
        {
        ...
        public ActionResult ExportExcelPaymentPending(RetPaymentListResponse model)
        {
            ...
            List<RetPaymentListForExp> lstproduct = objlst.Select(x => new RetPaymentListForExp()
            {
            ...
            var objlstvoucher = _paymentservice.GetAllRetailExpensesVoucherPaymentList(model.From, model.To, model.AreaID, model.UserID, model.CustomerID, model.DaysofWeek);
        ...
        // 11 Sep 2020 Piyush Limbani
        [HttpPost]
        public PartialViewResult ViewExpensesVoucherPaymentPendingList(RetVoucherPaymentListResponse model)
        {
            ViewBag.CashOption = _commonservice.GetAllCashOption();
            List<RetVoucherPaymentListResponse> objModel = _paymentservice.GetAllRetailExpensesVoucherPaymentList(model.From, model.To, model.AreaID, model.UserID, model.CustomerID, model.DaysofWeek);
            return PartialView(objModel);
        }

        [HttpPost]
        public ActionResult SaveExpenseVoucherPayment(List<RetVoucherPaymentListResponse> data)
        {
            if (Request.Cookies["UserID"] == null)
            {
                Request.Cookies["UserID"].Value = null;
        ...
        //Add By Dhruvik
        public ActionResult ChequeReturnEntryList()
        {
            ViewBag.SalesPersonList = _commonservice.GetRoleWiseUserList();
            ViewBag.AreaList = _commonservice.GetAllRetAreaList();
            ViewBag.Customer = _commonservice.GetActiveRetCustomerName(0);
        ...
        [HttpPost]
        public PartialViewResult ViewChequeReturnEntryList(RetCheckReturnEntryListResponse model)
        {
            List<RetCheckReturnEntryListResponse> objModel = _paymentservice.GetAllRetCheckReturnList(model);
            return PartialView(objModel);
        }

        [HttpPost]
        public ActionResult SaveRetReturnCheque(List<RetPaymentForCheckReturn> data)
        {
            bool respose = false;
            if (Request.Cookies["UserID"] == null)
            {
            ...
            else
            {
                foreach (var item in data)
                {
                    bool res = _paymentservice.IsCheckBounceOnRetPayment(item.PaymentID, item.OrderID, item.InvoiceNumber, Math.Abs(item.BounceAmount), item.ChequeReturnCharges);
                }
                long SessionUserID = Convert.ToInt64(Request.Cookies["UserID"].Value);
                respose = _paymentservice.SaveRetReturnCheque(data, SessionUserID);
            }
            return Json(respose, JsonRequestBehavior.AllowGet);
        }
        //Add By Dhruvik
